...
| `pnpm audit:docs` | Run the doc-quality audit checklist (iter-149 codification of `AGENTS.md § Doc-Quality Audit Checklist` grep blocks via `scripts/audit-docs.ts`; iter-150 wired into CI; iter-151 added 7th class for AGENTS.md ↔ runner self-parity; iter-161 added 8th class for `**N-package matrix**` total ↔ canonical 3-cohort breakdown sum parity; 8 drift classes + 1 cross-file parity check; exits non-zero on real drift; CI-gated since iter 150). |
