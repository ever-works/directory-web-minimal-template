...
- **All 28 .specify/ feature specs** complete and verified against code (added `q22-mobilemenu-ct.md` in iteration 108; added `q27-mobilemenu-empty-items-coverage.md` in iteration 123, ✅ RESOLVED in iteration 124; added `q28-eslint-10-upgrade.md` in iteration 129, OPEN — Option A chosen [DEFAULT]). The Q22→Q27 saga (CT migration + V8 coverage merge + per-file gate hard-fail + outlier-branch closure) is fully closed; every Q22-arc question (Q22, Q23, Q24, Q25, Q26, Q27) reports ✅ RESOLVED in `docs/questions.md` (Q27 footer flipped iter 129 — iter-125 health-check missed it), and the merged `pnpm coverage` pipeline writes a per-package report at branches **100% (233/233)**. **Q28 (iter 129)** retires the ESLint 9 → 10 deferral marker carried in iters 123/125-128 — pre-investigation finds the upgrade reduces to a single one-line peer-range bump (`packages/eslint-config/package.json` `"eslint": "^9.0.0"` → `"^10.0.0"`) plus a `pnpm install` + `pnpm lint` verification round; flat config since project inception, Node v24 already in CI, `@typescript-eslint@^8.59.0` peer-range covers ESLint 10, zero source usages of any removed/changed ESLint 10 API surface. Execution iteration estimated at 30-45 min single-iteration walltime.
...
- **All dependencies at latest versions** (Astro 6.1.9, Preact 10.29.1, Tailwind 4.2.4, TS 6.0.3, Prettier 3.8.3, Vitest 4.1.5, postcss 8.5.12, @typescript-eslint 8.59.0, Playwright 1.59.1, monocart-coverage-reports 2.12.11, monocart-reporter 2.10.1, vitest-monocart-coverage 4.0.2 — added iteration 119; matrix re-verified iter 123 / 125 / 126 / 127. Iter 127 surfaces a single delta inside the caret range: `isomorphic-git@1.37.5 → 1.37.6` (patch bump, caret-resolvable at the `^1.37.5` pin — next `pnpm install` adopts; deferred until a code-touching iteration runs `pnpm install` for any other reason)
