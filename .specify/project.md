...
- **All 26 .specify/ feature specs** complete and verified against code (added `q22-mobilemenu-ct.md` in iteration 108); **`q27-mobilemenu-empty-items-coverage.md` SPECIFIED in iteration 123** (carry from iteration 122 "Next Steps") — full feature spec + 4-step execution plan + Q27 entry in `docs/questions.md` (status `OPEN — Option A.1 chosen [DEFAULT]`); zero source/test/config changes — pure-doc iteration; targets the 3-branch outlier in `MobileMenu.tsx` focus-trap (B1 = `focusable.length === 0` early return + B2/B3 = non-boundary Tab fall-through). Default Option A.1 = synthetic `KeyboardEvent('keydown', { key: 'Tab' })` dispatch via `page.evaluate` to bypass the iter-120 CT-host-page focus-attribution race; A.3 (`v8 ignore next` pragma — verify monocart syntax first) is the contingency. Phase 0 smoke-test gate is the entry point for the execution iteration. Total estimated effort to close: 1.5-2 hours across 1-2 future iterations. Closing Q27 lifts MobileMenu to ~97-100% per file and the aggregate to ~99.6% (vs the current 91.89% / 98.72% — both already above the 80% per-file gate).
