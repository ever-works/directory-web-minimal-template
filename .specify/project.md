...
- **V8 code coverage**: **15/16 packages at 100% branch coverage**; `@ever-works/ui` is intentionally at **70.53% branch (158/224)** as the iteration-115 pre-merge state — the three CT-migrated components (`FilterBar.tsx`, `LayoutSwitcher.tsx`, `MobileMenu.tsx`) are no longer excluded from `packages/ui/vitest.config.ts` `coverage.exclude` (Phase 2 of Q22 follow-up #3), so they now appear in the include set with 0% Vitest coverage (Vitest never executes them). The CT runner still measures them via `monocart-reporter` (iteration 114 Phase 1) — `packages/ui/coverage/ct/raw-v8.json` carries 9 source-mapped entries at 84.88% branches / 100% functions / 97.18% lines on the CT subgraph, including all three components. Phase 3 introduces a `pnpm coverage` merge command that combines `coverage-final.json` (Vitest) + `raw-v8.json` (CT) into a single `coverage-summary.json`; once that lands, the per-package number returns to ≥99% branches and the Phase 2 70.53% intermediate state is no longer the published number.
