...
10. Total test coverage: **1137 Vitest unit tests** across 74 Vitest test files, 16 suites, 16 packages, **plus 28 Playwright Component Tests** (16 for `FilterBar` in `packages/ui/src/__tests__/ct/filter-bar.ct.test.tsx`, 12 for `LayoutSwitcher` in `packages/ui/src/__tests__/ct/layout-switcher.ct.test.tsx`) — total 1165 across both runners. The CT split was introduced in iteration 105 (Q22, FilterBar) and extended in iteration 107 (Q23, LayoutSwitcher) to bypass the Worker-IPC crash that prevented the original Vitest+jsdom files from running on Windows + Node 24 (see `docs/plans/q22-playwright-ct.md` and `docs/questions.md` Q22/Q23).
